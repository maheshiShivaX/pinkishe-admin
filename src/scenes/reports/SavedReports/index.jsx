...
const SavedReports = () => {
  ...
  const {
    ...
    stateDistrictWiseReport = { data: [], summary: null },
    machineWiseDispenseRefill = { data: [], summary: null },
    avgConsumptionReport = [],
    dispenseReport = { data: [], summary: null },

    machineWiseLoading,
    stateDistrictWiseLoading,
    avgConsumptionLoading,
    machineWiseDispenseRefillLoading,
  ...
  const { pdfData, pdfSummary, reportTitle } = useMemo(() => {
    if (currentType === "machine_wise_dispense") {
      return {
        pdfData: machineWiseReport?.data || [],
        pdfSummary: machineWiseReport?.summary || null,
    ...
    if (currentType === "state_district_wise_dispense") {
      return {
        pdfData: stateDistrictWiseReport?.data || [],
        pdfSummary: stateDistrictWiseReport?.summary || null,
    ...
    if (currentType === "machine_wise_dispense_refill") {
      return {
        pdfData: machineWiseDispenseRefill?.data || [],
        pdfSummary: machineWiseDispenseRefill?.summary || null,
    ...
    if (currentType === "dispense_report") {
      return {
        pdfData: dispenseReport?.data || [],
        pdfSummary: dispenseReport?.summary || null,
      ...
      <Box m="20px">
        <Header
          title={
            role === "user" && type === "user"
              ? "My Reports"
              : ((role === "superadmin" || role === "admin") && type === "user")
              ? "Users Report"
              : "Saved Reports"
          }
                  ...
                  <tbody>
                    {pdfData?.length > 0 ? (
                      ...
                      <tr>
                        <td
                          colSpan={
                            Object.keys(includeColumns).filter((k) => includeColumns[k]).length + 1
                          }
                          style={{
                            textAlign: "center",
                            padding: "16px",
